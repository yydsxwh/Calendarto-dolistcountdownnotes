...
1. 生产 `ACCOUNT_CLIENT_SECRET` 仍空时，BFF 登录返回 503，网页与 App 都无法完成真实 OIDC。App 代码路径已接好。
2. 正式上架需所有者提供签名密钥、隐私政策、商店图标；仓库不放密钥。
3. 仪器测试 `TodaySmokeTest` 需要模拟器 / 真机，本构建环境无系统镜像，未跑 UI 仪器测试。
