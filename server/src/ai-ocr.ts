...
import { isSpreadsheetName, parseSpreadsheet, spreadsheetAsText } from './spreadsheet'
...
export async function recognizeWithPlatform(
  ...
  if (isSpreadsheetName(input.fileName, input.mimeType)) {
    const parsed = parseSpreadsheet(input.bytes, input.fileName)
    if (parsed && (parsed.courses.length > 0 || parsed.exams.length > 0)) {
      return normalizeOcrPayload({ ...parsed, model: 'deterministic-sheet' }, input.kind)
    }
    const text = spreadsheetAsText(input.bytes, input.fileName)
    if (!text.trim()) {
      throw new OcrError('no_result', '没有从这份表格里认出课程或考试。请检查表头是不是「周一」或「课程 / 星期 / 时间」。', 422)
    }
    return recognizeWithPlatform(platform, {
      ...input,
