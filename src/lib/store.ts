...
export function createTodo(title:string,extras:{dueDate?:string;dueTime?:string;priority?:Priority;remindMinutes?:number}={}):Todo{return{id:uid(),title:title.trim(),done:false,dueDate:extras.dueDate,dueTime:extras.dueTime,priority:extras.priority??'medium',remindMinutes:extras.remindMinutes??15,createdAt:Date.now()}}
