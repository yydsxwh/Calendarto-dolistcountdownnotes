...
export interface SelfScheduleItem { id: string; title: string; weekday: number; startTime: string; endTime: string; color: string; note?: string; createdAt: number }
...
export interface AppData { todos:Todo[]; countdowns:Countdown[]; notes:Note[]; courses:Course[]; exams:Exam[]; selfSchedules:SelfScheduleItem[]; reminderSettings:ReminderSettings; terms:Term[]; currentTermId?:string; timetableView:TimetableViewSettings; termStart?:string }
